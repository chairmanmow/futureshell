...
function JSONChatHack(usernum,jsonclient,host,port) {
	...
	this.join = function(target) {
		...
		if(msgcount == 0) {
			...
			var str = format("\1h\1bLast msg: %.2d:%.2d on %.2d/%.2d/%.4d\r\n", //this is  llmorphg crap hack
	...
	this.who = function(target) {
		...
		chan.messages.push(new Message(undefined,"Users in " + chan.name + ": " + uList.join(", ") + "\r\n",Date.now()));  //another llmorphg hack
		return chan.users;
	}
